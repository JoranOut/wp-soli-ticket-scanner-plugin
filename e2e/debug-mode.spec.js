 ...
 * `display_errors` untouched when `WP_DEBUG` is false, and then no diagnostic
 * of any severity — not even a fatal — reaches the page, so those assertions
 ...
 * `WP_DEBUG: true` is not enough — it never reaches the tests environment,
 * which is exactly where Playwright runs. Confusingly `WP_DEBUG_LOG` and
 * `WP_DEBUG_DISPLAY` *do* carry over, which makes such a config look correct.
 * This test fails loudly if that regresses instead of letting the diagnostics
...
test.describe( 'PHP diagnostics are visible in the test environment', () => {
	// Site Health is admin-only; reuse the shared session from auth.setup.js.
