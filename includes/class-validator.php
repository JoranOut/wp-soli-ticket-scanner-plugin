...
class Validator {
	...
	/**
	 * Validate a ticket by delegating to Eventin Pro's check_ticket_id().
	 *
	 ...
	 */
	public function validate_ticket( int $attendee_id, string $ticket_id, int $event_id ): array {
		...
		if ( 2 === $update_status ) {
			return array(
				'status'   => 'success',
				'message'  => $result['msg'] ?? __( 'Ticket valid. Checked in.', 'soli-ticket-scanner' ),
	...
	private function get_attendee_info( int $attendee_id ): array {
		...
		$ticket_type = get_post_meta( $attendee_id, 'etn_ticket_name', true );
